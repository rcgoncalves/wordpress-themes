...
function rcg_gray_scripts_styles() {
	...
	wp_enqueue_script('rcg-gray-navigation', get_template_directory_uri() . '/inc/navigation.js', array(), '20140816', true);
	wp_enqueue_style('rcg-gray-style', get_stylesheet_uri(), array(), '20140816');
	wp_enqueue_style('rcg-gray-ie', get_template_directory_uri() . '/inc/ie.css', array('rcg-gray-style'), '20140816');
...
if(!function_exists('rcg_gray_entry_meta')) :
function rcg_gray_entry_meta() {
	...
	if(is_single() && $tags_list) {
		$utility_text .= ' ' . __('Tagged with %3$s.', 'rcg-gray');
	}
...
function rcg_gray_fix_category_tag ($cat_output) {
	return str_replace(array('rel="category tag"', 'rel="category"'), '', $cat_output);
}
add_filter('the_category', 'rcg_gray_fix_category_tag');
